...
func (this *Profile) GetList() []*ProfileSingle {
	...
	stmt, err := db.GetDb().Prepare(`SELECT t4.name as mname,t3.lname,t3.date,t3.tips,t3.status,t3.inspect,t3.min_num,t3.max_num,t3.tag,t4.vid FROM (
	...
	for rows.Next() {
		...
		rows.Scan(&single.MName, &single.LName, &single.Date, &single.Tips, &single.Status, &single.Inspect, &single.MinNum, &single.MaxNum, &single.Tag, &single.Vid)
