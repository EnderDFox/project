...
var ProfilePanel = {
	...
	GetTheadHtml:function(){
		...
		$(ProfileData.DateList).each(function(k,d){
			if(toDay == d){
	...
	GetTbodyHtml:function(){
		...
		$(ProfileData.DateList).each(function(k,d){
			html+= '<td><ol>'
			$(ProfileData.DataMap[d]).each(function(k,v){
				html+= '<li>'
	...
	//画图测试
	DrawCanvas:function(){
		html = '<div style="width:auto;border:0px solid red;display:inline-block;">'
