//个人类
var ProfilePanelClass = /** @class */ (function () {
    function ProfilePanelClass() {
    }
    //入口协议
    ProfilePanelClass.prototype.Index = function () {
        WSConn.sendMsg(C2L.C2L_PROFILE_VIEW, {});
    };
    //绑定事件
    ProfilePanelClass.prototype.BindActions = function () {
        //指定图表的配置项和数据
    ...
    //建立内容
    ProfilePanelClass.prototype.CreateProfile = function () {
        /*
        //组合thead
    ...
    //组合thead
    ProfilePanelClass.prototype.GetTheadHtml = function () {
        var html = '<tr>';
        var toDay = Common.FmtDate(new Date());
        $.each(ProfileData.DateList, function (k, d) {
            if (toDay == d) {
    ...
    //组合tbody
    ProfilePanelClass.prototype.GetTbodyHtml = function () {
        var html = '<tr>';
        $.each(ProfileData.DateList, function (k, d) {
            html += '<td><ol>';
    ...
    //画图测试
    ProfilePanelClass.prototype.DrawCanvas = function () {
        var html = '<div style="width:auto;border:0px solid red;display:inline-block;">';
