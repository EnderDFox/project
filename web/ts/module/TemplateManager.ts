...
class TemplateManagerClass {
    ...
    L2C_ModeEditName(data) {
        var modes = this.DataModes;
        var index = ArrayUtil.IndexOfByKey(modes, FieldName.Tmid, data.Tmid);
        if (index > -1) {
    ...
    L2C_LinkAdd(link: TplLinkSingle) {
        ...
        var modes = this.DataModes;
        var index = ArrayUtil.IndexOfByKey(modes, FieldName.Tmid, link.Tmid);
        if (index == -1) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var j = 0; j < mode.Links.length; j++) {
                ...
                if (tlink.Tlid == data.Tlid) {
                    ...
                    var index = ArrayUtil.IndexOfByKey(tlink.Children, FieldName.Tlid, data.Tlid);
                    if (index > -1) {
    ...
    L2C_LinkEditSort(data: L2C_TplLinkEditSort) {
        ...
        if (data.ParentTlid == 0) {
            ...
            var parentTlink: TplLinkSingle = ArrayUtil.FindByKey(mode.Links, FieldName.Tlid, data.ParentTlid);
            if (parentTlink) {
    ...
    GetTplLinkSingleByTlid(tlid: number): TplLinkSingle {
        ...
        for (var i = 0; i < len; i++) {
            var mode = modes[i]
            var index = ArrayUtil.IndexOfByKey(mode.Links, FieldName.Tlid, tlid)
            if (index > -1) {
            ...
            Loader.LoadVueTemplate(this.AuePath, "EditTplModeList", (tpl: string) => {
                this.vue_editTplModeList = new Vue({
                    ...
                    methods: {
                        ...
                        onEditName(e, Tmid) {
                            var modes = this.modes
                            var index = ArrayUtil.IndexOfByKey(modes, FieldName.Tmid, Tmid);
                            if (index > -1) {
                                var mode = modes[index]
    ...
    ShowEditTplModeDetail(e, showTmid: number, parentTlink: TplLinkSingle = null) {
        ...
        var mode: TplModeSingle;
        modeIndex = ArrayUtil.IndexOfByKey(modes, FieldName.Tmid, showTmid);
        if (modeIndex > -1) {
        ...
        var linkIndex
        if (parentTlink != null) {
            linkIndex = ArrayUtil.IndexOfByKey(mode.Links, FieldName.Tlid, parentTlink.Tlid);
            if (linkIndex > -1) {
            } else {
                    ...
                    onChangeDid: (e, tlid: number) => {
                        this.ShowMenuDepartment(e, (newDid: DidField) => {
                            if (tlid == -1) {//这是新建 里的部门选择
                                ...
                                var index = ArrayUtil.IndexOfByKey(_vue.linkList, FieldName.Tlid, tlid)
